...
export function Header(){return <>
  ...
  <header className="header">
    ...
    <nav aria-label="Main navigation"><Link href="/goods">Goods</Link><Link href="/about">The label</Link><Link href="/licence">Licensing</Link><Link href="/contact">Contact</Link></nav>
    <Link className="order-link" href="/order">Orders</Link>
...
export function Catalogue({products,unavailable=false}:{products:Product[];unavailable?:boolean}) {
  ...
  return <section className="catalogue" id="goods">
    ...
    {!filtered.length&&<div className="empty"><h3>Nothing filed here.</h3><p>Try a different search or reset the filters.</p><button className="button" onClick={()=>filter('All goods','','All tones')}>Reset filters</button></div>}
  </section>;
}
