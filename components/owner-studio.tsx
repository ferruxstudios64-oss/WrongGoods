...
async function api<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error((data as { error?: string }).error || 'The request could not be completed. Try again.');
  return data as T;
}
...
export function OwnerStudio() {
  ...
  function load() {
    return api<{products:Draft[]}>('/api/owner/products')
      .then(data=>{setProducts(data.products);setError('');})
      .catch(e=>setError(e instanceof Error?e.message:'Owner access is unavailable.'))
  ...
  return <main id="main" className="owner-studio">
    <div className="owner-heading"><div><span className="eyebrow">WRONGGOODS / OWNER STUDIO</span><h1>THE WORKBENCH.</h1><p>Prepare the goods. Check the details. Release when ready.</p></div>{products && <div className="owner-header-links"><a className="text-link" href="/owner/inbox">Inbox &amp; subscribers</a><a className="text-link" href="/cdn-cgi/access/logout">Sign out</a></div>}</div>
    ...
    {!products ? <section className="owner-gate"><span className="eyebrow">PRIVATE ACCESS</span><h2>{busy ? 'Checking owner access...' : 'Owner sign-in required.'}</h2><p>This studio requires your approved Cloudflare Access account. Open the configured owner address and sign in with your authorised email.</p><p>If you are running locally, owner access stays locked until the authentication service and private storage are configured.</p><button className="button" disabled={busy} onClick={() => void load()}>Check access again</button><Link className="text-link" href="/">Back to the storefront</Link></section> : <div className="owner-layout">
